...
test('sidecar defaults to official DeepSeek and reads the key from the environment', async () => {
  ...
  assert.match(source, /OFFICIAL_PROVIDER = 'deepseek'/)
  ...
  assert.match(source, /input:[\s\S]*-\s*image/)
  assert.match(source, /officialVisionProviderLines/)
...
test('API proxy refuses model credential writes so keys stay in the environment', async () => {
  ...
  assert.match(source, /rewriteTextOnlyOfficialProvider/)
