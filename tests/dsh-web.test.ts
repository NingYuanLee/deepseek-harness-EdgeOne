...
test('settings and model welcome preferences persist through Host even off loopback', async () => {
  ...
  assert.match(models, /officialProvided: "Models come from the official DeepSeek API\. The key is read only from DEEPSEEK_API_KEY\."/)
  assert.match(models, /officialProvided: "模型由 DeepSeek 原厂提供，API Key 仅从环境变量 DEEPSEEK_API_KEY 读取。"/)
