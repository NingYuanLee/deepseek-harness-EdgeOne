...
test('workspace UI shows a single cloud workspace without switching', async () => {
  ...
  assert.match(conversation, /"placeholder.workspace": "正在准备沙箱…"/)
  assert.match(conversation, /"placeholder.workspace": "Preparing the sandbox…"/)
  ...
  assert.match(conversation, /onRequestWorkspace: \(\) => \{\}/)
  assert.match(conversation, /dsh-makers:open-sandbox/)
  assert.match(conversation, /dsh-makers:insert-reference/)
  assert.match(conversation, /mentionFromSandbox/)
  assert.match(workspace, /sandbox workspace adopt failed/)
  assert.match(workspace, /this\.workspaces\.create\(\{ path \}\)/)
  assert.match(workspace, /if \(this\.clearArchivedCurrent\(\)\) initial = "waiting"/)
  assert.match(workspace, /const sessionsExpanded = true;/)
  assert.doesNotMatch(workspace, /const sessionsExpanded = expandedSessionGroups\.includes\(group\.key\)/)
...
test('page chrome keeps a GitHub link and a sandbox file browser', async () => {
  const html = await readFile(new URL('../index.html', import.meta.url), 'utf8')
  assert.match(html, /dsh-makers-actions/)
  assert.match(html, /dsh-makers-chrome/)
  assert.match(html, /dsh-makers-sandbox/)
  assert.match(html, /\/api\/sandbox\/files/)
  assert.match(html, /\/api\/sandbox\/file/)
  assert.match(html, /沙箱文件/)
  assert.match(html, /dsh-makers:insert-reference/)
  assert.match(html, /mention: "引用"/)
  assert.match(html, /upload: "上传"/)
  assert.match(html, /delete: "删除"/)
  assert.match(html, /dsh-makers-sandbox-confirm/)
  assert.match(html, /confirm-ok/)
  assert.match(html, /askDelete/)
  assert.doesNotMatch(html, /window\.confirm/)
  assert.match(html, /dsh-makers-sandbox-mention/)
  assert.match(html, /cite\.textContent = "@"/)
...
test('generated API routes expose static files the Makers scanner accepts', async () => {
  ...
  const workspaceCreate = await readFile(new URL('../agents/api/workspace/create.ts', import.meta.url), 'utf8')
  const sessionCreate = await readFile(new URL('../agents/api/session/create.ts', import.meta.url), 'utf8')
  assert.match(workspaceCreate, /export async function onRequest/)
  assert.match(workspaceCreate, /\.\.\/_proxy\.ts/)
  assert.match(sessionCreate, /export async function onRequest/)
  assert.match(generator, /directoryPicker\/pick/)
  assert.match(generator, /workspace\/create/)
  assert.match(generator, /session\/create/)
  assert.match(generator, /sandbox\/files/)
  assert.match(generator, /sandbox\/file/)
  assert.match(generator, /fileReferences\/list/)
  const sandboxFiles = await readFile(new URL('../agents/api/sandbox/files.ts', import.meta.url), 'utf8')
  assert.match(sandboxFiles, /export async function onRequest/)
  assert.match(generator, /settings\/update/)
